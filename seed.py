...
# We use the app context to tell Python which database to talk to
with app.app_context():
    
    # 1. This line forces Python to build the universities.db file right now!
    db.create_all()
    
    # 2. Create the Universities (using the exact images from your HTML)
    uni1 = University(
    ...
    # 4. Commit the changes (save them permanently)
    db.session.commit()

    print("SUCCESS: Database built and 3 Universities added!")